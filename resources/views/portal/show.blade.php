    ...
    <div class="relative z-10 flex flex-col min-h-screen">
        ...
        <div class="h-48 bg-surface-1 border-b border-border relative overflow-hidden flex items-center justify-center">
            ...
            <div class="absolute top-6 left-6 z-20 opacity-0 animate-[fadeIn_0.3s_ease-out_forwards]">
                <a href="{{ route('portal.index') }}" class="inline-flex items-center gap-2 px-3 py-1.5 rounded-none bg-surface-2 hover:bg-surface-3 text-text-secondary hover:text-text-primary text-[11px] font-mono uppercase tracking-[0.08em] transition-colors duration-150 border border-border">
        ...
        <main class="flex-1 max-w-7xl mx-auto w-full px-6 lg:px-8 -mt-16 relative z-10 pb-24">
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div class="lg:col-span-4 space-y-6 opacity-0 animate-[fadeIn_0.3s_ease-out_50ms_forwards]">
                    <div class="bg-surface-1 border border-border rounded-none p-6">
                        <div class="flex flex-col items-center text-center mb-6">
                            @if($team->logo)
                                <div class="w-20 h-20 rounded-none bg-surface-2 border border-border mb-4 overflow-hidden flex items-center justify-center">
                                    <img src="{{ asset('storage/' . $team->logo) }}" alt="{{ $team->name }}" class="w-full h-full object-cover">
                                </div>
                            @else
                                <div class="w-20 h-20 rounded-none bg-surface-2 flex items-center justify-center text-3xl font-display font-medium text-text-primary border border-border mb-4">
                                    {{ substr($team->name, 0, 1) }}
                                </div>
                            @endif
                            <h1 class="text-2xl font-display font-medium text-text-primary mb-2 tracking-tight">{{ $team->name }}</h1>
                            <div class="inline-flex items-center px-2 py-0.5 rounded-none bg-bg border border-border text-[10px] font-mono text-text-muted uppercase tracking-[0.08em] mb-6">
                                Verified Hub
                            </div>
                    ...
                    <div class="bg-surface-1 border border-border rounded-none overflow-hidden opacity-0 animate-[fadeIn_0.3s_ease-out_100ms_forwards]">
                        <div class="px-6 py-4 border-b border-border bg-surface-2 flex items-center gap-3">
                            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="square" stroke-linejoin="miter" class="text-text-secondary"><path d="M4 19.5v-15A2.5 2.5 0 0 1 6.5 2H20v20H6.5a2.5 2.5 0 0 1-2.5-2.5Z"/><path d="M8 7h6"/><path d="M8 11h8"/></svg>
                            <h2 class="text-[11px] font-mono text-text-primary uppercase tracking-[0.08em]">Knowledge Base</h2>
                        </div>
                        <div class="divide-y divide-border">
                            @foreach($articles as $article)
                                <a href="{{ route('portal.articles.show', [$team, $article]) }}" class="block p-4 hover:bg-surface-2 transition-colors duration-150 group">
                                    <div class="flex items-center justify-between gap-4">
                                        <div class="flex-1 min-w-0">
                                            <h3 class="text-[13px] font-medium text-text-primary group-hover:text-accent transition-colors duration-150 truncate mb-0.5">
                                                {{ $article->title }}
                                            </h3>
                                            <div class="flex items-center gap-2 text-[9px] font-mono text-text-muted uppercase tracking-[0.08em]">
                                                <span>{{ number_format($article->view_count) }} views</span>
                                            </div>
                                        </div>
                                        <div class="flex-shrink-0 text-text-muted group-hover:text-accent transition-colors duration-150 transform group-hover:translate-x-1">
                ...
                <div class="lg:col-span-8 opacity-0 animate-[fadeIn_0.3s_ease-out_100ms_forwards] space-y-8">
                    <!-- Success Message -->
                    @if (session('status'))
                        <div class="bg-success/10 border border-success/20 text-success p-4 rounded-none flex items-center gap-3">
                            <div class="w-6 h-6 rounded-none bg-success/20 flex items-center justify-center flex-shrink-0 border border-success/20">
                            ...
                            <form action="{{ route('portal.show', $team) }}" method="GET" class="flex flex-col sm:flex-row gap-3">
                                <div class="relative group">
                                    ...
                                    <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Search..."
                                        class="bg-bg border border-border rounded-none text-[12px] py-2 pl-9 pr-3 text-text-primary w-full sm:w-48 placeholder-text-muted focus:border-accent focus:ring-0 focus:shadow-[0_0_0_3px_var(--color-accent-glow)] transition-all outline-none">
                                </div>

                                <div class="flex items-center gap-2">
                                    <span class="text-[10px] font-mono text-text-muted uppercase tracking-[0.08em]">Sort</span>
                                    <select name="sort" onchange="this.form.submit()" class="bg-bg border border-border rounded-none text-[11px] font-mono uppercase tracking-[0.08em] focus:border-accent focus:ring-0 focus:shadow-[0_0_0_3px_var(--color-accent-glow)] py-2 pl-2 pr-6 text-text-primary cursor-pointer outline-none transition-all">
                                            ...
                                            @if($ticket->tags->isNotEmpty())
                                                <div class="mt-3 flex flex-wrap gap-1.5">
                                                    @foreach($ticket->tags as $tag)
                                                        <span class="inline-flex items-center px-2 py-0.5 rounded-none text-[10px] font-mono uppercase tracking-[0.08em] bg-bg text-text-secondary border border-border">
                                                            {{ $tag->name }}
                                                        </span>
                                                    @endforeach
                                                </div>
                    ...
                    <div class="space-y-5">
                        <div class="space-y-1.5">
                            ...
                            <input type="text" name="title" id="title" required placeholder="What needs attention?"
                                class="w-full bg-surface-2 border border-border rounded-none px-4 py-2.5 text-text-primary placeholder-text-muted focus:border-accent focus:ring-0 focus:shadow-[0_0_0_3px_var(--color-accent-glow)] transition-all outline-none text-[13px]">
                        </div>
                        ...
                        <div class="space-y-1.5">
                            ...
                            <textarea name="description" id="description" rows="5" required placeholder="Describe the problem in high detail..."
                                class="w-full bg-surface-2 border border-border rounded-none px-4 py-3 text-text-primary placeholder-text-muted focus:border-accent focus:ring-0 focus:shadow-[0_0_0_3px_var(--color-accent-glow)] transition-all outline-none resize-none text-[13px] leading-relaxed"></textarea>
                        </div>

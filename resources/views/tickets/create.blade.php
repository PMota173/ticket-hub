<x-layouts.app title="Create Ticket - {{ config('app.name', 'Ticket Hub') }}" sidebar="team">
    <div class="max-w-3xl mx-auto">
        ...
        <div class="bg-surface-1 border border-border rounded-none p-8 opacity-0 animate-[fadeIn_0.3s_ease-out_50ms_forwards]">
            <form action="{{ route('tickets.store', $team) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                @csrf
                ...
                <div class="space-y-6">
                    ...
                    <div class="space-y-1.5 relative group overflow-hidden">
                        <label for="attachments" class="block text-[11px] font-mono text-text-muted uppercase tracking-[0.08em] mb-2">Attachments</label>
                        <div class="flex items-center gap-4">
                            ...
                            <span id="create-file-count" class="text-[11px] font-mono text-text-muted">No files selected</span>
                        </div>
                        <input type="file" name="attachments[]" id="attachments" multiple class="absolute inset-0 w-full h-full opacity-0 cursor-pointer pointer-events-none" onchange="document.getElementById('create-file-count').textContent = this.files.length > 0 ? this.files.length + ' file(s) selected' : 'No files selected'">

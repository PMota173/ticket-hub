<aside class="w-72 bg-slate-900 border-r border-slate-800 flex flex-col h-screen sticky top-0 font-sans">
    ...
    <div class="p-6">
        <a href="/" class="flex items-center gap-3 mb-8 group">
            ...
            <span class="text-xl font-bold tracking-tight text-white group-hover:text-blue-400 transition-colors">Ticket Hub</span>
        </a>

        <!-- Navigation -->
        <nav class="space-y-1">
            <a href="{{ route('dashboard') }}"
               class="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-all duration-200 group relative overflow-hidden {{ request()->routeIs('dashboard') ? 'text-white bg-blue-500/10' : 'text-slate-400 hover:text-white hover:bg-slate-800' }}">
            ...
            <a href="{{ route('portal.index') }}"
               class="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-all duration-200 group relative overflow-hidden {{ request()->routeIs('portal.index') ? 'text-white bg-blue-500/10' : 'text-slate-400 hover:text-white hover:bg-slate-800' }}">
                @if(request()->routeIs('portal.index'))
                    <div class="absolute left-0 top-0 bottom-0 w-1 bg-blue-500 rounded-r-full"></div>
                @endif
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="{{ request()->routeIs('portal.index') ? 'text-blue-400' : 'text-slate-500 group-hover:text-slate-300' }} transition-colors"><circle cx="12" cy="12" r="10"/><polygon points="16.24 7.76 14.12 14.12 7.76 16.24 9.88 9.88 16.24 7.76"/></svg>
                Explore
            </a>
            
            <a href="{{ route('teams.index') }}"
               class="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-all duration-200 group relative overflow-hidden {{ request()->routeIs('teams.index') ? 'text-white bg-blue-500/10' : 'text-slate-400 hover:text-white hover:bg-slate-800' }}">
                @if(request()->routeIs('teams.index'))
                    <div class="absolute left-0 top-0 bottom-0 w-1 bg-blue-500 rounded-r-full"></div>
                @endif
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="{{ request()->routeIs('teams.index') ? 'text-blue-400' : 'text-slate-500 group-hover:text-slate-300' }} transition-colors"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"></path><circle cx="9" cy="7" r="4"></circle><path d="M22 21v-2a4 4 0 0 0-3-3.87"></path><path d="M16 3.13a4 4 0 0 1 0 7.75"></path></svg>
                My Teams
            </a>

            <a href="{{ route('teams.create') }}"
               class="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-all duration-200 group relative overflow-hidden {{ request()->routeIs('teams.create') ? 'text-white bg-blue-500/10' : 'text-slate-400 hover:text-white hover:bg-slate-800' }}">
                @if(request()->routeIs('teams.create'))
                    <div class="absolute left-0 top-0 bottom-0 w-1 bg-blue-500 rounded-r-full"></div>
                @endif
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="{{ request()->routeIs('teams.create') ? 'text-blue-400' : 'text-slate-500 group-hover:text-slate-300' }} transition-colors"><circle cx="12" cy="12" r="10"/><path d="M8 12h8"/><path d="M12 8v8"/></svg>
                Create Team
            </a>

            <a href="{{ route('my-invitations.index') }}"
               class="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-all duration-200 group relative overflow-hidden {{ request()->routeIs('my-invitations.index') ? 'text-white bg-blue-500/10' : 'text-slate-400 hover:text-white hover:bg-slate-800' }}">
                @if(request()->routeIs('my-invitations.index'))
                    <div class="absolute left-0 top-0 bottom-0 w-1 bg-blue-500 rounded-r-full"></div>
                @endif
                <div class="relative">

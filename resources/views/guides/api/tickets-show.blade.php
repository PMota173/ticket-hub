...
@section('content')
    <h1 class="text-3xl md:text-4xl font-black text-white mb-6">GET /tickets/{id}</h1>
    <p class="text-slate-400 text-lg mb-8 leading-relaxed">Retrieve a specific ticket by ID. The response does not include comments.</p>

    <h2 class="text-xl font-bold text-white mb-4">Request Examples</h2>
    <div id="block-show" class="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-xl mb-10">
        <div class="flex border-b border-slate-800 bg-slate-950">
            <button onclick="switchTab('block-show', 'curl')" class="btn-curl tab-btn px-6 py-3 text-xs font-bold uppercase tracking-widest text-blue-400 border-b-2 border-blue-500 bg-blue-500/10">cURL</button>
            <button onclick="switchTab('block-show', 'js')" class="btn-js tab-btn px-6 py-3 text-xs font-bold uppercase tracking-widest text-slate-400 border-b-2 border-transparent hover:text-white">JavaScript</button>
            <button onclick="switchTab('block-show', 'py')" class="btn-py tab-btn px-6 py-3 text-xs font-bold uppercase tracking-widest text-slate-400 border-b-2 border-transparent hover:text-white">Python</button>
        </div>
        <div class="panel-curl code-panel p-6 overflow-x-auto">
            <pre class="text-sm font-mono text-slate-300">curl -X GET "{{ url('/api/v1/tickets/1') }}" \
...
<pre class="text-blue-300">
{
  ...
  <span class="text-green-400">"updated_at"</span>: <span class="text-orange-300">"2026-01-29T12:00:00.000Z"</span>
}</pre>
    </div>
@endsection

@section('pagination')
    <a href="{{ route('guides.api', 'tickets-list') }}" class="flex items-center gap-2 text-sm font-bold text-slate-500 hover:text-white transition-colors">
    ...
    <a href="{{ route('guides.api', 'tickets-create') }}" class="flex items-center gap-2 text-sm font-bold text-blue-400 hover:text-white transition-colors">
        Next: POST /tickets

...
@section('content')
    <h1 class="opacity-0 animate-[fadeIn_0.3s_ease-out_forwards] text-3xl font-display font-medium text-text-primary mb-6">GET /tickets</h1>
    <p class="text-text-secondary text-base mb-8 leading-relaxed">Retrieve all tickets for the robot's team.</p>

    <h2 class="text-base font-display font-medium text-text-primary mb-4">Request Examples</h2>
    <div id="block-list" class="bg-surface-2 border border-border rounded-none overflow-hidden shadow-xl mb-10">
        <div class="flex border-b border-border bg-bg">
            <button onclick="switchTab('block-list', 'curl')" class="btn-curl tab-btn px-6 py-3 text-[11px] font-mono uppercase tracking-[0.08em] text-accent border-b-2 border-accent bg-surface-2">cURL</button>
            <button onclick="switchTab('block-list', 'js')" class="btn-js tab-btn px-6 py-3 text-[11px] font-mono uppercase tracking-[0.08em] text-text-secondary border-b-2 border-transparent hover:text-accent-hover">JavaScript</button>
            <button onclick="switchTab('block-list', 'py')" class="btn-py tab-btn px-6 py-3 text-[11px] font-mono uppercase tracking-[0.08em] text-text-secondary border-b-2 border-transparent hover:text-accent-hover">Python</button>
        </div>
        <div class="panel-curl code-panel p-6 overflow-x-auto">
            <pre class="text-[13px] font-mono text-text-primary">curl -X GET "{{ url('/api/v1/tickets') }}" \
...
@section('pagination')
    <a href="{{ route('guides.show', 'api-reference') }}" class="flex items-center gap-2 text-[11px] font-mono uppercase tracking-[0.08em] text-text-muted hover:text-accent-hover transition-colors">
    ...
    <a href="{{ route('guides.api', 'tickets-show') }}" class="flex items-center gap-2 text-[11px] font-mono uppercase tracking-[0.08em] text-accent hover:text-accent-hover transition-colors">
        Next: GET /tickets/{id}
